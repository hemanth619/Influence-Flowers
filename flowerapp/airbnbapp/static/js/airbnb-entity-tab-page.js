function initTabPageEntity(flower_id, data, container_width, num_petals, entity_type) {
    ...
    function get_info_node_coordinates(context, i) {
        ...
        var angles = get_angles(Math.PI, 0, parent['property_type'].length);
        var angle = angles[i];
    ...
    function get_angles(start, end, num_petals) {
        var result = [], step = 0;
        if (num_petals > 1)
            step = (end - start) / (num_petals - 1);
        else
            step = 0;
    ...
    function get_petal_node_coordinates(num_petals) {
        var angles = get_angles(Math.PI, 0, num_petals);
    ...
    function greyOutBars(num_petals) {
        d3.selectAll(".bar-chart")
            .filter(function (d) {
                if (d != undefined) return d.bloom_order <= num_petals;
            })
    ...
    bar_chart = svg.append("g").selectAll("rect")
        ...
        .attr("x", function (d) {
            if (d.type == "in"){ 
                return x(d.neighbourhood) + x.bandwidth() / 2;
            }else{ 
    ...
    svg.append("g")
        .attr("transform", "translate(100," + (height + 70) + ")")
    ...
    svg.append("text")
        .attr("transform", "translate(" + (container_width / 2) + "," + (height + 100) + ")")
        .style("text-anchor", "start")
        .text("Top " + top_numbers + " of total " + data["links"].length + " " + entity_type);


    svg.append("g")
        .attr("transform", "translate(" + ((center[0] - container_width * 0.3) + 100) + "," + (height - 50) + ")")
    ...
    svg.append("defs").selectAll("marker")
        ...
        .attr("d", "M0,-5L10,0L0,5")
        .style("fill", function (d) {
            if (d.type == "in") return colors(0.2);
            else return colors(0.8);
        });
    ...
    // petal info nodes
    info_nodes = info_nodes_g.selectAll("circle .info-node-circle")
        .data(nodes)
        .enter()
        .append('g')
        .each(function (d, i) {
    ...
    petal_node_text = petal_node_text_g.selectAll("text")
        .data(nodes)
        .enter().append("text")
        .attr("id", function (d) {
            return d.id;
    ...
    info_node_text = info_node_text_g.selectAll("text .info-node-text")
        .data(nodes)
        .enter()
        .append('g')
        .each(function (d, i) {
    ...
    info_node_circle_center_text = info_node_circle_center_text_g.selectAll("text .info-node-text")
        .data(nodes)
        .enter()
        .append('g')
        .each(function (d, i) {
    ...
    info_node_edge_link = info_node_edge_link_g.selectAll("path .info-node-link")
        .data(nodes)
        .enter()
        .append('g')
        .each(function (d, i) {
            if (i > 0 && i <= num_petals) {
                d3.select(this).selectAll('path .info-node-link')
                    ...
                    .attr("class", function (d) {
                        return "info-node-link " + d.type;
                    })
                    .attr("node-name", function (d) {
                        return d3.select(this.parentNode).datum()['name'];
                    })
                    .style("stroke-width", function (d) {
                        var movies = d3.select(this.parentNode).datum()['property_type'];
                        var min = movies[0]["value"];
                        var max = movies[0]["value"];
                        for (var i = 0; i < movies.length; i++) {
                            if (movies[i]["value"] < min) {
                                min = movies[i]["value"];
                            }
                            if (movies[i]["value"] > max) {
                                max = movies[i]["value"];
                            }
                        }
                        // normalize
                        var normalized_weight = (d.value - min) / (max - min)
                        if ((d.value - min) === 0 && d.value != 0) {
    ...
    // flower edges
    flower_edge_link = flower_edge_link_g.selectAll("path .flower-edge-link")
        .data(links)
        .enter().append("path")
        .attr("id", function (d) {
            return d.id;
